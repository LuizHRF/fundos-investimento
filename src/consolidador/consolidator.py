...
- composicao_carteira.csv: Portfolio composition (CDA data)
"""
from pathlib import Path
from typing import Tuple, Optional

from .parsers import parse_rcvm175, parse_cda
from .merger import merge_fund_with_class, prepare_output
from .exporter import export_fundos, export_carteira, print_summary
...
def consolidate(force: bool = False) -> Tuple[Path, Optional[Path]]:
    ...
    3. Parse portfolio composition (CDA)
    4. Export both files
    ...
    # Step 3: Parse portfolio composition (CDA)
    print()
    carteira_df = parse_cda(use_cache=use_cache, force=force)

    # Step 4: Export
    fundos_path = export_fundos(output_df)
    carteira_path = export_carteira(carteira_df)

    # Summary
    print_summary(output_df, carteira_df)
